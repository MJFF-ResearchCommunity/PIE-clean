...
def test_load_biospecimen(caplog, tmp_path):
    ...
    # Default call excludes only the standard files, but the return is also
    # missing the batch-corrected project 151 that we haven't mocked.
    ...
    for data_type in ALL_DATA:
        if data_type in ["standard_files", "project_151_pQTL_CSF_batch_corrected"]:
    ...
    caplog.clear()
    # Explicitly exclude nothing, but non-mocked project 151 still missing
    data = load_biospecimen_data(DATA_DIR, exclude=[])
    assert "Will exclude the following projects" not in caplog.text
    for data_type in ALL_DATA:
        if data_type == "project_151_pQTL_CSF_batch_corrected":
            assert data_type not in data.keys()
        else:
            assert data_type in data.keys()
...
def test_load_project_151(caplog, tmp_path):
    # First up, not batch-corrected
    key = 'project_151_pQTL_CSF'
    ...
    expected = "Successfully processed Project_151"
    msg = [r.message for r in caplog.records if expected in r.message]
    assert len(msg) == 1
    # Logging shows the data was pivoted into wide format
    assert "2 rows, 8 columns" in msg[0]
    # There should be no batch-corrected
    assert 'project_151_pQTL_CSF_batch_corrected' not in data_dict
    # Unwrap df
    df = data_dict[key]
    ...
    # Next, we haven't mocked the files for batch_corrected
    key = 'project_151_pQTL_CSF_batch_corrected'
    data_dict = load_biospecimen_data(DATA_DIR,
                                      exclude=[d for d in ALL_DATA if d != key])
    ...
    assert data_dict == {} # No non-batch corrected loaded either
    ...
    data_dict = load_biospecimen_data(tmp_path,
                                      exclude=[d for d in ALL_DATA if d != key])
    expected = f"is missing PATNO column"
    msg = [r for r in caplog.records if expected in r.message]
    assert len(msg) == 2 # 2 files
    assert msg[0].levelname == "WARNING"
    expected = "No files were successfully loaded"
    msg = [r for r in caplog.records if expected in r.message]
    assert len(msg) == 1
...
def test_load_metabolomic_lrrk2(caplog, tmp_path):
    ...
    key = 'metabolomic_lrrk2'
    data_dict = load_biospecimen_data(DATA_DIR,
                                      exclude=[d for d in ALL_DATA if d != key])
    # Logging shows that the files were found correctly
    expected = "Successfully processed Metabolomic"
    msg = [r for r in caplog.records if expected in r.message]
    assert len(msg) == 1
    # Logging shows the data was pivoted into wide format
    assert "2 rows, 8 columns" in msg[0].message
    # Make sure there's no CSF
    assert "metabolomic_lrrk2_csf" not in data_dict
    # Unwrap the df
    df = data_dict[key]
    assert df.shape[0] == 2 # ...and the output matches the logging
    # PATNOs from each file have been merged together (ints here)
    ...
    # Clear the caplog so we can be sure of what was generated by this call
    caplog.clear()
    # Next, test the CSF load 
    key = 'metabolomic_lrrk2_csf'
    data_dict = load_biospecimen_data(DATA_DIR,
                                      exclude=[d for d in ALL_DATA if d != key])
    # Logging shows that the files were found correctly
    expected = "Successfully processed Metabolomic"
    ...
    assert "2 rows, 6 columns" in msg[0].message
    # There should be no non-CSF
    assert "metabolomic_lrrk2" not in data_dict
    # Unwrap the df
    df = data_dict[key]
    assert df.shape[0] == 2 # Check the output matches the logging above
    ...
    # And we have V06 events only
    assert "V04" not in df["EVENT_ID"].tolist()
...
def test_load_project_177(caplog, tmp_path):
    ...
    data_dict = load_biospecimen_data(DATA_DIR,
                                      exclude=[d for d in ALL_DATA if d != key])
    # Logging shows that the files were found correctly
    expected = "Successfully processed Project_177"
    msg = [r.message for r in caplog.records if expected in r.message]
    assert len(msg) == 1
    # Logging shows the data was pivoted into wide format
    assert "2 rows, 8 columns" in msg[0]
    # Unwrap the df
    df = data_dict[key]
    assert df.shape[0] == 2 # Confirm the output matches the logging
    # PATNOs from each file have been merged together (ints here)
    assert "9999" in df["PATNO"].tolist()
    assert "9998" in df["PATNO"].tolist()
...
def test_load_and_join_biospec_files(caplog, tmp_path):
    ...
    # Columns from duplicates appear
    assert "ORIG_ENTRY" in df.columns.tolist()
    assert "ORIG_ENTRY_1" in df.columns.tolist()
    assert "ORIG_ENTRY_2" in df.columns.tolist()
    assert "OFF_SCHEDULE_2" in df.columns.tolist()
    assert "OFF_SCHEDULE_1" not in df.columns.tolist() # doesn't appear in 2nd df
    # Correct merging of duplicate values for <PATNO, EVENT_ID> pairs
    assert "PAG_NAME" in df.columns
    assert "PAG_NAME_1" in df.columns
    assert "PAG_NAME_2" in df.columns
    assert "MUTRSLT" in df["PAG_NAME"].tolist()
    assert "SKBIO" in df["PAG_NAME_1"].tolist()
    assert "LUMBAR" in df["PAG_NAME_2"].tolist()
    assert (df[df["EVENT_ID"]=="BL"]["PAG_NAME_1"]=="SKBIO").all()
    assert (df[df["EVENT_ID"]=="BL"]["PAG_NAME_2"]=="LUMBAR").all()
    assert df[df["EVENT_ID"]=="BL"]["PAG_NAME"].isnull().all()
    assert (df[df["EVENT_ID"]=="GMU"]["PAG_NAME"]=="MUTRSLT").all()
    assert df[df["EVENT_ID"]=="GMU"]["PAG_NAME_1"].isnull().all()
    assert df[df["EVENT_ID"]=="GMU"]["PAG_NAME_2"].isnull().all()
